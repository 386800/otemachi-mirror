...
/* op-materials-v6: タブ / カリキュラム進行線 / カリキュラム開閉 */
(function(){
  'use strict';
  if(!document.querySelector('.ml, .op-md'))return;
  var reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  /* --- ARIA tabs (model output) --- */
  var tablist=document.querySelector('.ml-tablist');
  if(tablist){
    ...
    function select(t,focus){
      tabs.forEach(function(x){
        ...
        var p=panelFor(x);
        if(p){
          if(on){p.hidden=false;if(!reduced){p.classList.remove('ml-panel--in');void p.offsetWidth;p.classList.add('ml-panel--in');}}
          else{p.hidden=true;}
        }
      });
      if(focus)t.focus();
  ...
  var line=document.querySelector('.ml-track__line');
  if(line){
    if(reduced){line.style.transform='scaleY(1)';line.classList.add('is-lit');}
    else{
      var track=line.parentElement;
      var upd=function(){
        var r=track.getBoundingClientRect();
        var vh=window.innerHeight||800;
        var total=r.height+vh*0.4;
        var passed=Math.min(Math.max(vh*0.6-r.top,0),total);
        var p=total>0?Math.min(1,passed/total):0;
        line.style.transform='scaleY('+p.toFixed(3)+')';
        line.classList.toggle('is-lit',p>0.02);
      };
  ...
  /* --- curriculum "すべての章を見る" (detail page) --- */
  document.addEventListener('click',function(e){
    var b=e.target.closest?e.target.closest('[data-more]'):null;
    if(b){var box=b.closest('.op-md-cur');if(box)box.classList.add('is-open');}
  });
})();

/* op-lightbox-v1b: 図表クリックで拡大表示（capture段階でカード遷移と分離） */
(function(){
  var SEL = '.ml-shot__f img, .ml-panel__fig img';
  function openLb(src, alt){
    var lb = document.createElement('div');
    lb.className = 'op-lb';
    lb.setAttribute('role', 'dialog');
    lb.setAttribute('aria-label', (alt || '図表') + 'の拡大表示');
    var im = document.createElement('img');
    im.src = src; im.alt = alt || '';
    var x = document.createElement('button');
    x.type = 'button'; x.className = 'op-lb__x'; x.textContent = '閉じる ✕';
    var cap = document.createElement('div');
    cap.className = 'op-lb__cap'; cap.textContent = alt || '';
    lb.appendChild(im); lb.appendChild(x); lb.appendChild(cap);
    function close(){
      lb.classList.remove('is-on');
      document.body.classList.remove('op-lb-lock');
    ...
    document.body.classList.add('op-lb-lock');
  ...
  document.addEventListener('click', function(e){
    ...
    if (!img || img.closest('a') || img.closest('.op-lb')) { return; }
    e.preventDefault();
    e.stopPropagation();
