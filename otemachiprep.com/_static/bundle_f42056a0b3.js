...
(function(){
var links=[].slice.call(document.querySelectorAll('a.lib-item')).filter(function(a){return /otemachi-prep_article_/.test(a.getAttribute('href')||'')});
if(!links.length)return;var KEY='opCourseRead';var read={};try{read=JSON.parse(localStorage.getItem(KEY)||'{}')}catch(e){}
var bar=document.createElement('div');bar.id='courseProgress';
bar.innerHTML='<div class="wrap" style="display:flex;align-items:center;gap:14px"><strong style="font-size:.84rem;color:var(--navy);white-space:nowrap">読了 <span id="cpN">0</span> / '+links.length+'</strong><div style="flex:1;height:6px;background:#EDF1F6;border-radius:3px;overflow:hidden"><div id="cpBar" style="height:100%;width:0;background:linear-gradient(90deg,var(--navy),#C9A961);transition:width .35s"></div></div><button id="cpReset" type="button" style="font-size:.7rem;color:#8496AC;background:none;border:none;cursor:pointer;white-space:nowrap">リセット</button></div>';
var main=document.getElementById('main');main.insertBefore(bar,main.firstChild);
function save(){try{localStorage.setItem(KEY,JSON.stringify(read))}catch(e){}}
function refresh(){var n=0;links.forEach(function(a){var k=a.getAttribute('href');var on=!!read[k];var m=a.querySelector('.cp-mark');m.textContent=on?'✓':'';m.className='cp-mark'+(on?' on':'');if(on)n++;});
document.getElementById('cpN').textContent=n;document.getElementById('cpBar').style.width=(100*n/links.length)+'%';}
links.forEach(function(a){var m=document.createElement('span');m.className='cp-mark';m.title='クリックで読了を記録';
m.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();var k=a.getAttribute('href');if(read[k]){delete read[k]}else{read[k]=1}save();refresh();});
a.insertBefore(m,a.firstChild);});
document.getElementById('cpReset').addEventListener('click',function(){if(confirm('読了記録をリセットしますか？')){read={};save();refresh();}});
refresh();})();
